...
import { Loader2, Plus, Trash2, MessageCircleQuestion, GripVertical } from "lucide-react";
...
export function ServicesManager({ services }: { services: ServiceData[] }) {
  ...
  const [editingId, setEditingId] = useState<string | null>(null); // id del servicio o "new"
  ...
  async function handleSave() {
    ...
    try {
      if (editingId === "new") {
        await axios.post("/api/services", toPayload(form));
      } else {
        await axios.patch(`/api/services/${editingId}`, toPayload(form));
      }
      closeEditor();
      router.refresh();
    } catch (err: any) {
